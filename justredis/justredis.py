        ...
        if isinstance(sock.getsockname(), tuple):
            ...
            if tcpkeepalive:
                ...
                if platform == 'linux':
                    ...
                    sock.ioctl(socket.SIO_KEEPALIVE_VALS, (1, tcpkeepalive * 1000, tcpkeepalive // 3 * 1000))
        self.commands = deque()
        self.closed = False
...
class Command:
    __slots__ = '_data', '_database', '_encoder', '_decoder', '_throw', '_retries', '_enqueue', '_server', '_got_result', '_result', '_asking', '_event', '_ready_to_await'
...
class PubSub:
    ...
    async def register(self, instance, channels=None, patterns=None):
        registered_channels, registered_patterns = self._registered_instances.setdefault(instance, (set(), set()))
        if channels:
